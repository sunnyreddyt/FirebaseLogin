...
public class BankDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       /* if (abUtil.isConnectingToInternet()) {
        ...
        new BankDetails().execute();

    }


    private class BankDetails extends AsyncTask<String, Integer, String> {
            ...
            try {
                ...
                if (json.length() > 0) {
                    ...
                    try {
                        ...
                        if (jsonObjectMain.has("Return code") && jsonObjectMain.getString("Return code").equalsIgnoreCase("200")) {

                            String responseMessage = jsonObjectMain.getString("Response Msg");

                            JSONArray jsonArray = jsonObjectMain.getJSONArray("bank_details");

                            LinearLayoutManager linearLayoutManager = new LinearLayoutManager(BankDetailsActivity.this, LinearLayoutManager.VERTICAL, false);
                            usersRecycleView.setLayoutManager(linearLayoutManager);
                            usersRecycleView.setHasFixedSize(true);
                            BankDetailsAdapter bankDetailsAdapter = new BankDetailsAdapter(BankDetailsActivity.this, jsonArray);
                            usersRecycleView.setAdapter(bankDetailsAdapter);
